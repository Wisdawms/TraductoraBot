...
def translate_en_es(message):
    ...
    if message.reply_to_message is None and not len(message.text.split()) > 1:
        print("263 en es")
        bot_reply_msg = bot.send_message(message.chat.id, f"{message.from_user.first_name}, please reply to this message with the EN text or voice note you wish to translate to {to_lang.upper()}", reply_markup=telebot.types.ForceReply(selective=True), reply_to_message_id=message.message_id)
        bot.register_for_reply(bot_reply_msg, handle_trans_reply, to_lang, flag_msg=message, from_lang=from_lang, from_locale=None, to_locale=None)
        return
    try:
        if message.reply_to_message is not None: # if this has a reply
...
def translate_es_en(message):
    ...
    if message.reply_to_message is None and not len(message.text.split()) > 1:
        print("263 es en")
        bot_reply_msg = bot.send_message(message.chat.id, f"{message.from_user.first_name}, please reply to this message with the ES text or voice note you wish to translate to {to_lang.upper()}", reply_markup=telebot.types.ForceReply(selective=True), reply_to_message_id=message.message_id)
        bot.register_for_reply(bot_reply_msg, handle_trans_reply, to_lang, flag_msg=message, from_lang=from_lang, from_locale=None, to_locale=None)
        return
    try:
        if message.reply_to_message is not None: # if this has a reply
...
def translate_ar_es(message):
    ...
    if message.reply_to_message is None and not len(message.text.split()) > 1:
        print("263 es en")
        bot_reply_msg = bot.send_message(message.chat.id, f"{message.from_user.first_name}, please reply to this message with the AR text or voice note you wish to translate to {to_lang.upper()}", reply_markup=telebot.types.ForceReply(selective=True), reply_to_message_id=message.message_id)
        bot.register_for_reply(bot_reply_msg, handle_trans_reply, to_lang, flag_msg=message, from_lang=from_lang, from_locale=None, to_locale=None)
        return
    try:
        if message.reply_to_message is not None: # if this has a reply
